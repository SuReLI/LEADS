...
from utils.never_give_up import NGU
import math
...
class NGU_A(Algorithm): 
    def __init__(self, algo, env_class, env_params, N, gamma, lr_theta, lr_q, lr_starts, tau, policy_frequency, ngu_frequency,  target_frequency, noise, alpha, autotune, n_episode, batch_size, device, n_env, wandb_project_name, wandb_entity, seed):
        ...
        self.ngu_frequency = ngu_frequency
        ...
        # ngu module 
        self.ngu_module = NGU(self.env.observation_space.shape[0], self.env.action_space.shape[0])
        self.ngu_optimizer =torch.optim.Adam(list(self.ngu_module.rnd.parameters()) + list(self.ngu_module.parameters()), lr=lr_theta)
    ...
    def train(self):
        ...
        for epoch in range(self.N):
            ...
            for episode in range(self.n_episode):
                    ...
                    for t in range(self.env.max_steps):
                        ...
                        with torch.no_grad():
                            for idx in range(self.n_env) : 
                                ri,s_dm_v = self.ngu_module.r_i(torch.tensor(s[idx], dtype=torch.float32, device=self.device), torch.tensor(self.replay_buffer_states[idx][-1],dtype=torch.float32, device=self.device), s_dm[idx])
                                r[idx] = ri 
                                s_dm[idx] = s_dm_v
                        ...
                        # update ngu
                        ngu_loss = self.update_ngu(self.global_step)
                        # global step 
                        self.global_step+=1
                    ...
                    print('coverage : ', self.coverage(), 'sample : ', self.sample_nb, 'ngu_loss : ', ngu_loss)
            # wandb 
    ...
    def update_ngu(self, global_step): 
        if global_step > self.learning_starts and global_step % self.ngu_frequency == 0:
            (s, a, r, d, s_next) = self.sample(self.batch_size)
            loss = self.ngu_module.loss(s,a,s_next)
            self.ngu_optimizer.zero_grad()
            loss.backward()
            self.ngu_optimizer.step()
            return loss.item()
        else : 
            return 0.0
    ...
    def plot_maze(self, epoch):
        ...
        with torch.no_grad():
            uncertainty = list(self.ngu_module.uncertainty_measure(torch.tensor(s, dtype=torch.float32).to(self.device)).cpu().numpy())
        # ax1
...
if __name__=='__main__': 
    ...
    algo = 'ngu'
    wandb_entity = 'ngu'
    algo = NGU_A(algo, env_class, env_params, N, n_skill, z_dim, gamma, lr_theta, lr_q, lr_starts, tau, policy_frequency, target_frequency, noise, alpha, autotune, n_episode, batch_size, device, n_env, wandb_project_name, wandb_entity, seed)
    algo.train()
