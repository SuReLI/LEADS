...
class Codebook(nn.Module):
    def __init__(self, n, d, device='cpu', std=1, learning_rate=1e-4, algo ='lds', w=1.0):
        ...
        self.w=w
        self.codebook_vectors = nn.Parameter(self.sample_orthonormal_vectors(n,d)*self.w) if algo=='lds' else nn.Parameter(torch.eye(n,n).to(device)*self.w)
    ...
    def sample_orthonormal_vectors(self, n, d):
        ...
        random_matrix = torch.randn(d, n).to(self.device)
