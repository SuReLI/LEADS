...
class Whopper(gym.Env):
    def __init__(self, name='hopper', render = False, path_video = './video', name_prefix = 'hopper'):
        ...
        self.observation_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=((self.sim.data.qpos.shape[0]+
                                                                                  ...
                                                                                  self.sim.data.get_body_xpos("foot").shape[0] ),))
        self.render_mode = 'rgb_array'

    def reset(self):
        self.sim.reset()
        self.sim.step()
        s0 = np.concatenate([self.sim.data.qpos.flat.copy(),self.sim.data.qvel.flat.copy(),self.sim.data.get_body_xpos("torso").copy(), self.sim.data.get_body_xpos("foot").copy()])
        return s0, {'pos':np.concatenate([self.sim.data.get_body_xpos("torso").copy()[:], self.sim.data.get_body_xpos("foot").copy()[:]])}
    ...
    def step(self, action):
        ...
        for _ in range(self.frame_skip):
            self.sim.step()
        s = np.concatenate([self.sim.data.qpos.flat.copy(),self.sim.data.qvel.flat.copy(),self.sim.data.get_body_xpos("torso").copy(), self.sim.data.get_body_xpos("foot").copy()])
        return s, 0, False, False, {'pos':np.concatenate([self.sim.data.get_body_xpos("torso").copy()[:], self.sim.data.get_body_xpos("foot").copy()[:]])}
