...
from smm_algorithm.discriminator import Discriminator
import math
...
class SMM(Algorithm): 
    def __init__(self, algo, env_class, env_params, N, n_skill, z_dim, gamma, lr_theta, lr_q, lr_starts, tau, policy_frequency, target_frequency, noise, alpha, autotune, n_episode, batch_size, device, n_env, wandb_project_name, wandb_entity, seed, featurize):
        ...
        # ensure z==n
        # discriminator
        self.discriminator = Discriminator(self.env.observation_space.shape[0],self.n_skill, self.env.name, featurize,
                                           latent_dim=16, hidden_dim=256, beta=0.5).to(self.device)
        ...
        for epoch in range(self.N):
            for episode in range(self.n_episode):
                    ...
                    for t in range(self.env.max_steps):
                        ...
                        with torch.no_grad():
                            p_s_z =(torch.log(torch.sum(self.discriminator(torch.tensor(s, dtype=torch.float32, device=self.device))*z_omega, dim = 1)) - np.log(1/self.n_skill)).cpu().numpy()
                            p_s = self.discriminator.loss_vae_n(torch.tensor(s, dtype=torch.float32, device=self.device)).cpu().numpy()
    ...
    def update_discriminator(self, epoch): 
        ...
        loss_z = self.loss_discriminator(z_pred,z.argmax(dim=1))
        loss_vae = self.discriminator.loss_vae(s)
        loss = loss_z + loss_vae
