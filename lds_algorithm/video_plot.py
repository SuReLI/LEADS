...
from env.mujoco.record_video import RecordVideo
...
for i in range(n_skill):
    env = RecordVideo(env_class(render=True), './video', video_length=env_unwrapped.max_steps, name_prefix = 'skill_'+str(i))
    print('codebook : ', codebook.codebook_vectors[i])
    s = env.reset()[0] 
