...
class SoftQNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, z_dim, device='cpu'):
        super().__init__()
        self.fc1 = nn.Linear(state_dim + action_dim + z_dim, 256)
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, z_dim, action_space):
        super().__init__()
        self.fc1 = nn.Linear(state_dim + z_dim, 256)
        self.fc2 = nn.Linear(256+z_dim, 256)
        self.fc_mean = nn.Linear(256+z_dim, action_dim)
        self.fc_logstd = nn.Linear(256+z_dim, action_dim)
        # action rescaling
    ...
    def forward(self, x, z):
        ...
        # x = torch.cat([x, z], 1)
        x = F.relu(self.fc1(x)) 
        x = torch.cat([x, z], 1) #residual connection
        x = F.relu(self.fc2(x))
        x = torch.cat([x, z], 1) #residual connection
        mean = self.fc_mean(x)
    ...
    def get_action(self, x, z, successor_sampling=False, eps = None):
        mean, log_std = self(x, z)
        std = log_std.exp()
        normal = torch.distributions.Normal(mean, std) if not successor_sampling else torch.distributions.Normal(mean, std)
        ...
        log_prob = log_prob if not successor_sampling else normal.log_prob(action).sum(1, keepdim=True)
