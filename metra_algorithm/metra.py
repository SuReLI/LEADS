...
class METRA(Algorithm): 
    def __init__(self, algo, env_class, env_params, N, n_skill, z_dim, gamma, lr_theta, lr_q, lr_starts, tau, k_update, policy_frequency, target_frequency, noise, alpha, autotune, n_episode, batch_size, device, n_env, wandb_project_name, wandb_entity, seed, featurize):
        ...
        # ensure z==n
        # discriminator
        self.discriminator = Discriminator(self.env.observation_space.shape[0],z_dim, self.env.name, featurize).to(self.device)
        ...
        # sac 
        # codebook 
        self.codebook = Codebook(n=n_skill, device=device, algo='lds', w= 5.0)
        # actor
    ...
    def train(self):
        ...
        for epoch in range(self.N):
            ...
            for step_u in range(self.k_update): 
                # update actor 
                self.update_sac(step_u)
                # update disciminator 
                self.update_discriminator()
            # plot 
            self.plot_maze(epoch) if self.maze_type else self.plot_mujoco(epoch)
            self.save_model(epoch) if (epoch % 1 ==0) else None
            # wandb 
    ...
    def update_sac(self, step ): 
        ...
        # make r here 
        delta_phi = (self.discriminator(s_next).detach()- self.discriminator(s).detach())
        r_ = 0 
        for i in range(self.n_skill) : r_ += -1/(self.n_skill-1)*torch.sum(delta_phi*self.codebook.codebook_vectors[i], dim = 1) 
        r = (1+1/(self.n_skill+1))*torch.sum(delta_phi*z.detach(), dim = 1) + r_
        with torch.no_grad():
        ...
        if step % self.policy_frequency == 0:  
            for _ in range(
        ...
        if step % self.target_frequency == 0:
    ...
    def update_discriminator(self): 
        (s, a, r, d, s_next, z) = self.sample(self.batch_size)
        phi_s = self.discriminator(s)
        phi_s_next = self.discriminator(s_next)
        lsd_loss =  - ((phi_s_next - phi_s)* z).sum(dim=1).mean()
        metra_regu = -(self.discriminator.lambda_metra.detach()*torch.min(self.discriminator.eps*torch.ones_like(d), torch.ones_like(d) - ((phi_s_next - phi_s)* z).sum(dim=1))).mean()
        loss_phi = lsd_loss + metra_regu
        loss_lambda = (self.discriminator.lambda_metra*torch.min(self.discriminator.eps*torch.ones_like(d), torch.ones_like(d) - ((phi_s_next - phi_s)* z).sum(dim=1) ).detach()).mean()
        loss = loss_phi + loss_lambda
